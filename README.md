...
| Package | Role |
|---|---|
| `pi-claude-bridge` | The `claude-bridge` provider: Claude models through the local Claude Code login. Pinned to the fork commit `wayne930242/pi-claude-bridge@31891e9` on `weihung-integration`, which merges two upstream PRs: 200K twins such as `claude-200k-opus-5-5` beside each 1M model ([#131](https://github.com/elidickinson/pi-claude-bridge/pull/131)), so main and complex tiers run Opus 5.5 1M while other tiers run 200K; and Claude subscription usage in the footer plus `/claude-usage` ([#132](https://github.com/elidickinson/pi-claude-bridge/pull/132)). |
